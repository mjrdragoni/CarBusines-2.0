...
uses
  ...
  System.ImageList, Vcl.ImgList, Vcl.ComCtrls, Vcl.ToolWin, frxGradient;

type
  TFrmRelUsers = class(TFrmPatternRel)
    NomeDe: TLabeledEdit;
...
var
  FrmRelUsers: TFrmRelUsers;

implementation

{$R *.dfm}

procedure TFrmRelUsers.btncancelrelClick(Sender: TObject);
begin
  inherited;
    edtCodigoDe.Clear;
    edtCodigoAte.Clear;
...
procedure TFrmRelUsers.btnprintClick(Sender: TObject);
Var StrLiga : String;
begin
  inherited;
    StrLiga := 'where ';
    ...
    begin
      ...
      try
        ...
        if NomeDe.Text <> '' then
        begin
          Add(StrLiga+'login >= '''+NomeDe.Text+'''');
          StrLiga := 'and ';
        end;
          if NomeAte.Text <> '' then
          Add(StrLiga+'login <= '''+NomeAte.Text+'zzz''');
          case RadioGroup1.ItemIndex of
            0 : Add('order by id');
            1 : Add('order by login');
          end;
    end;
    FDQuery1.Open;
