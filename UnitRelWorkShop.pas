...
type
  TFrmRelWorkShop = class(TFrmPatternRel)
    NomeDe: TLabeledEdit;
...
var
  FrmRelWorkShop: TFrmRelWorkShop;

implementation

{$R *.dfm}

procedure TFrmRelWorkShop.btncancelrelClick(Sender: TObject);
begin
  inherited;
edtCodigoDe.Clear;
    edtCodigoAte.Clear;
...
procedure TFrmRelWorkShop.btnprintClick(Sender: TObject);
Var StrLiga : String;
begin
  inherited;
StrLiga := 'where ';
    ...
    begin
      ...
      try
        ...
        if NomeDe.Text <> '' then
        begin
          Add(StrLiga+'conpany_name >= '''+NomeDe.Text+'''');
          StrLiga := 'and ';
        end;
          if NomeAte.Text <> '' then
          Add(StrLiga+'conpany_name <= '''+NomeAte.Text+'zzz''');
          case RadioGroup1.ItemIndex of
            0 : Add('order by id');
            1 : Add('order by conpany_name');
          end;
    end;
    FDQuery1.Open;
...
procedure TFrmRelWorkShop.ToolButton1Click(Sender: TObject);
begin
  inherited;
 close;
end;
