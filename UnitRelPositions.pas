...
type
  TFrmRelPosition = class(TFrmPatternRel)
    NomeDe: TLabeledEdit;
...
var
  FrmRelPosition: TFrmRelPosition;

implementation

{$R *.dfm}

procedure TFrmRelPosition.btncancelrelClick(Sender: TObject);
begin
  inherited;
    edtCodigoDe.Clear;
    edtCodigoAte.Clear;
...
procedure TFrmRelPosition.btnprintClick(Sender: TObject);
Var StrLiga : String;
begin
  inherited;
    StrLiga := 'where ';
    ...
    begin
      ...
      try
        ...
        if NomeDe.Text <> '' then
        begin
          Add(StrLiga+'position >= '''+NomeDe.Text+'''');
          StrLiga := 'and ';
        end;
          if NomeAte.Text <> '' then
          Add(StrLiga+'position <= '''+NomeAte.Text+'zzz''');
          case RadioGroup1.ItemIndex of
            0 : Add('order by id');
            1 : Add('order by position');
          end;
    end;
    FDQuery1.Open;
